...
<HTML>
    ...
    <input type="hidden" name="searchConnectionName" value="%value searchConnectionName%">
        ...
        <td colspan=2>
            <ul>
            <li><a id="Return_to_%value displayName%_Connections" href="ListResources.dsp?adapterTypeName=%value -urlencode adapterTypeName%&searchConnectionName=%value -urlencode searchConnectionName%&dspName=.LISTRESOURCES">Return to %value displayName% Connections
            </a><script>updateURL("Return_to_%value displayName%_Connections", "Return to %value displayName% Connections");</script></ul>
        </td>
        </tr>
...
<table class="tableView" width="100%">
        ...
        %ifvar mgdConnTypes%
            ...
            <a id="%value mgdConnFactoryDisplay%" href="../WmART/GetConnProperties.dsp?adapterTypeName=%value -urlencode adapterTypeName%&connectionFactoryType=%value -urlencode connectionFactoryType%&searchConnectionName=%value -urlencode ../searchConnectionName%&adapterPackageName=%value -urlencode ../adapterPackageName%">%value mgdConnFactoryDisplay%</a><script>updateURL("%value mgdConnFactoryDisplay%", "%value mgdConnFactoryDisplay%");</script>
			</td>
