...
<BODY onLoad="setNavigation('ListPollingNotifications.dsp', '%value encode(javascript) helpsys%', 'foo')">
    <form name="form" action="ListPollingNotifications.dsp" method="POST"
        ...
        <table width=100%>
            ...
            <tr><td colspan=2>
            <ul>
            <li><A id="return_to_%value displayName%_notifications" HREF="ListPollingNotifications.dsp?adapterTypeName=%value -urlencode adapterTypeName%&searchNotificationName=%value -urlencode searchNotificationName%&dspName=.LISTPOLLINGNOTIFICATIONS">Return to %value displayName% Notifications</A>
			<script>updateURL("return_to_%value displayName%_notifications", "Return to %value displayName% Notifications");</script>
            </ul>
            </td></tr>
        </table>
