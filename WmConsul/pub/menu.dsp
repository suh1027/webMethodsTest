...
function initMenu(firstImage) {
  ...
  <body class="menu" onload="initMenu('')">
    ...
    <table class="menuTable" width="100%" cellspacing="0" cellpadding="0" border="0">
      ...
      %scope param(expanded='true') param(text='Microservices')%
        %include ../../WmRoot/pub/menu/section-top.dsp%
          Microservices
        %include ../../WmRoot/pub/menu/section-bottom.dsp%
      %endscope%

      %scope param(section='Registry') param(text='Configuration') param(url='settings-registry.dsp')%
          ...
          %invoke wm.server.csrfguard:replaceSpecialCharacters%
            <script>
              ...
              if (is_csrf_guard_enabled && needToInsertToken) {
                ...
                document.write('<a id="a%value encode(htmlattr) url%" href="javascript:htmlform_menu_subelement_%value replacedString%.submit()"> Registry %ifvar target% ... %endif% </a>');
              } else {
                document.write('<a id="a%value encode(htmlattr) url%" target="%ifvar target%%value $host%%value target%%else%body%endif%" href="%value encode(javascript) url%"> Registry %ifvar target% ... %endif% </a>');
              }
            </script>
          %endinvoke%
          %endscope%
