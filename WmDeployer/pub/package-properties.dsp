...
function setPackageType(pkgType) {
	document.getElementById("deleteDurable").style.display = "none";
	...
	if (pkgType == "full") {
		...
		var action = document.getElementsByName('actionOnDeploy');
		for (var i = 0; i < action.length; i++) {
			if (action[i].checked && action[i].value == "activate") {
				document.getElementById("deleteDurable").style.display = "";
			}
		}
	}
	else {
...
function setAction(action) {
	if (action == "activate") {
		var package = document.getElementsByName('packageReleaseType');
		for (var i = 0; i < package.length; i++) {
			if (package[i].checked && package[i].value == "full") {
				document.getElementById("deleteDurable").style.display = "";
			}
		}
	}
		...
		<TR>
			...
			<TD><TABLE class="tableForm">
				...
				<TR>
					...
					%ifvar /defineAuthorized equals('true')%
						<INPUT onclick="setAction('activate');" type=radio name="actionOnDeploy" value="activate" 
						%ifvar actionOnDeploy equals('activate')% CHECKED %endif% >Activate
						<INPUT onclick="setAction('install');" type=radio name="actionOnDeploy" value="install"
						%ifvar actionOnDeploy equals('install')% CHECKED %endif% >Install Only
						<INPUT onclick="setAction('none');" type=radio name="actionOnDeploy" value="none"
				...
				<TR id="deleteDurable">
					<TD class="oddrow">Delete Universal Messaging Durable</TD>
					<TD colspan="2" class="oddrowdata-l">
					%ifvar /defineAuthorized equals('true')%
						<INPUT type=radio name="deleteUMDurable" value="true" 
						%ifvar deleteUMDurable equals('true')% CHECKED %endif% >Yes
						<INPUT type=radio name="deleteUMDurable" value="false"
						%ifvar deleteUMDurable equals('false')% CHECKED %endif% >No
					%else%
						%ifvar deleteUMDurable equals('true')% Yes %else% No %endif% 
					%endif%
					</TD>
				</TR>
