...
function setAllEmptyValues(chk){
       if (chk.checked){
		document.getElementById("allowEmptyValues").value="true";
		}
}
...
<TABLE width=100%>
	...
	<TR>
		...
		<INPUT type="hidden" name="allowEmptyValues" id="allowEmptyValues" value="%ifvar(allowEmptyValues)%%value allowEmptyValues%%else%false%endif%">
		<INPUT type="hidden" name="runtimeType" value='%value runtimeType%'>
		 %ifvar propertiesExists equals('true')%
		     
		     <label> <INPUT type="checkBox" id="allowEmptyValueschk" name="allowEmptyValueschk" onclick="setAllEmptyValues(this);" %ifvar allowEmptyValues equals('true')% checked %endif%>Allow Empty Values</label>   
	 ...
	 %loop properties%
		  ...
		  %endif%
				...
				<SCRIPT> writeTD("row-l");  </SCRIPT>
		           ...
		           %else%		           
		           		%ifvar propertyExistingValue equals('CONFLICTED VALUES')%
		           			<input style="border:solid 3px red;" id="%value ../propertyName%" name="%value ../propertyName%"  value=" "
			       				%ifvar ../isSecure equals('true')% 
								type="password"
			       				%endif%  size="40">
			       	   %else%
			       	   		<input id="%value ../propertyName%" name="%value ../propertyName%"  value="%value propertyExistingValue%"
			       				%ifvar ../isSecure equals('true')% 
								type="password"
			       				%endif%  size="40">
			       		%endif%
